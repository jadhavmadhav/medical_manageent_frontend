            ...
            {filteredProducts.map((product, index) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 cursor-pointer transition-all duration-150 border-b border-gray-100 last:border-b-0 ${isHighlighted
                ...
                >
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3
                        className={`font-semibold text-base mb-1 truncate ${isHighlighted ? "text-white" : "text-gray-900"
                          }`}
                      >
                        {product?.name}
                      </h3>
                      ...
                      <div className="flex items-center gap-1.5 mb-2">
                        <Building2
                          className={`w-3.5 h-3.5 ${isHighlighted ? "text-blue-100" : "text-gray-500"
                            }`}
                        />
                        <span
                          className={`text-sm ${isHighlighted ? "text-blue-100" : "text-gray-600"
                            }`}
                        >
                      ...
                      <div className="flex flex-wrap gap-3 text-xs">
                        <div className="flex items-center gap-1">
                          <Calendar
                            className={`w-3 h-3 ${isHighlighted ? "text-blue-200" : "text-gray-400"
                              }`}
                          />
                          <span
                            className={
                    ...
                    <div className="text-right flex-shrink-0">
                      <div
                        className={`font-bold text-lg mb-1 flex items-center justify-end gap-0.5 ${isHighlighted ? "text-white" : "text-gray-900"
                          }`}
                      >
                        <IndianRupee className="w-4 h-4" />
                        {product?.pricing?.sellingPerBaseUnit?.toFixed(2)}
                      </div>

                      <div
                        className={`text-xs font-medium px-2 py-1 rounded-full inline-block ${isHighlighted
                      ...
                      >
                        Stock: {product?.stock?.baseAvailable}
                      </div>
                    </div>
                  </div>
                </div>
