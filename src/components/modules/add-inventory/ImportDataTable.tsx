...
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Info, Upload, FileSpreadsheet, FileText } from "lucide-react";
import ImportButtons from "./ImportButtons";
import TemplateButtons from "./TemplateButtons";
import DataTable from "./DataTable";
import { toast } from "sonner";
...
const ImportDataTable = memo(
  ({ extraFieldsForProduct, data, setData }: ImportDataTableProps) => {
          ...
          reader.onload = (e) => {
            try {
              ...
              toast.success(`Successfully imported ${processedData.length} records`, {
                description: `${type.toUpperCase()} file processed`,
              });
              resolve();
            } catch (error) {
              console.error("Error parsing file:", error);
      ...
      <div className="flex flex-col h-full space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 p-1 bg-white border border-slate-200 rounded-xl shadow-sm">
            <span className="px-3 text-sm font-medium text-slate-600">Import:</span>
            <ImportButtons onFileUpload={handleFileUpload} />
          </div>
        ...
        {/* Stats Footer */}
        {data.length > 0 && (
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-slate-200">
            <div className="flex items-center gap-2">
              <FileSpreadsheet className="h-5 w-5 text-blue-600" />
              <span className="font-semibold text-slate-900">
                {data.length} {data.length === 1 ? "record" : "records"} loaded
              </span>
            </div>
            <button
              onClick={() => {
