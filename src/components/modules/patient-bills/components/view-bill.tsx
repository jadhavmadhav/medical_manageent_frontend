...
const BillItemRow = memo(({ item, bill }: { item: BillItem; bill: Bill }) => {
...
export const ViewBill = memo(({ bill, children }: ViewBillProps) => {
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent className="w-[95vw] max-w-[800px] p-6 rounded-2xl shadow-2xl">
        {/* Header */}
          ...
          {billItems.length > 0 ? (
            <div className="max-h-[400px] overflow-y-auto">
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-gray-100 border-b text-gray-700 uppercase text-xs z-10">
                  <tr>
                ...
                <tbody>
                  {billItems.map((item) => (
                    <BillItemRow key={item._id} item={item} bill={bill} />
                  ))}
                </tbody>
              </table>
            </div>
        ...
        <div className="flex justify-end mt-4">
          <div className="w-full max-w-[280px] bg-white border rounded-lg p-3 text-sm text-gray-700 shadow-sm">
            ...
            <div className="flex justify-between font-semibold text-base text-gray-900 border-t mt-2 pt-2">
              <span>Total:</span>
